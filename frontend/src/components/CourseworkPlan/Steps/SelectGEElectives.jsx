...
const SelectGEElectives = ({ onSelectionChange, courses = [], requiredCount = 5, loading = false }) => {
  ...
  useEffect(() => {
    ...
    console.log("GE Electives courses:", courses);
  ...
  // Function to notify parent of selection change
  const notifyParent = (selectedCoursesMap) => {
    // Extract only the selected courses data
    const selectedCoursesArray = Object.keys(selectedCoursesMap)
      .filter(id => selectedCoursesMap[id]?.selected)
      .map(id => ({
        id,
        year: selectedCoursesMap[id].year,
        semester: selectedCoursesMap[id].semester
      }));
    
    // Call the parent's onSelectionChange handler
    onSelectionChange && onSelectionChange(selectedCoursesArray);
  };
  ...
  // Handle changing the year for a selected course
  ...
  // Handle changing the semester for a selected course
  ...
  if (!courses || courses.length === 0) {
    return (
      <div className="text-center p-4 text-gray-500">
        No GE elective courses available
      </div>
    );
  }
