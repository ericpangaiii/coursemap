...
const CompactSemesterCard = ({ semester, courses, year, onGradeChange, hideDetailsButton = false }) => {
  ...
  const hasCourses = coursesState.length > 0;
  ...
  return (
    <>
      <Card className={`w-full ${isMidyear ? 'h-[160px]' : hasCourses ? 'h-[420px]' : 'h-[200px]'} flex flex-col`}>
        <CardHeader className="py-2 px-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-xs font-medium text-gray-600 dark:text-gray-300">
              {getSemesterName(semester)}
            </CardTitle>
            {!hideDetailsButton && coursesState.length > 0 && (
              <Button
                variant="ghost" 
                size="sm"
        ...
        <CardContent className={`px-2 pt-2 pb-0 flex-1 flex flex-col ${isMidyear ? 'max-h-[100px]' : hasCourses ? 'max-h-[360px]' : 'max-h-[140px]'}`}>
